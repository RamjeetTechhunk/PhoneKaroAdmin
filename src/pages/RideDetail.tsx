...
const RideDetail: React.FC = () => {
  ...
  const fetchRideDetail = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch ride details",
      );
    } finally {
      setLoading(false);
    }
        ...
        {loading ? (
          ...
          <div className="bg-white shadow rounded-lg overflow-hidden">
            <div className="px-6 py-4 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1">
                      <span
                        className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusBadgeColor(ride.rideStatus)}`}
                      >
                        {ride.rideStatus}
                      </span>
                    </p>
              ...
              {/* Driver Details */}
              {ride.driverDetails &&
                Object.keys(ride.driverDetails).length > 0 && (
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">
                      Driver Details
                    </h3>
                    <div className="grid grid-cols-2 gap-4">
                      {Object.entries(ride.driverDetails).map(
                        ([key, value]) => {
                          if (value == null || String(value) === "")
                            return null;
                          const strVal = String(value);
                          const isImageUrl =
                            key.toLowerCase() === "driverprofile" ||
                            (typeof value === "string" &&
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-900">
                        {ride.ambulance.AmbulanceType || "N/A"}
                      </p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">
                      ...
                      <p className="mt-1 text-sm text-gray-900">
                        {ride.ambulance.vehicleNumber || "N/A"}
                      </p>
                    </div>
              ...
              {/* Discount */}
              {ride.discount &&
                (ride.discount.coupon || ride.discount.value) && (
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">
                      Discount
                    </h3>
